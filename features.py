...
def get_features_labels(data_df, price_df, feature_type = None):
    ...
    for i in range(len(sorted_price_df)-1):
        ...
        feature_list.append(list(single_feature)) # may cause a bug
...
def get_single_value_features(data_df):
    interval_value = data_df['sentiment'].sum()
...
def load_list_from_file(input_file=None, type_func=int):
    ...
    with open(input_file, 'r') as ifile:
        ...
        for row in reader:
            if row == []:
                continue
            feature_list.append(list(map(type_func, row)))
    return feature_list
...
if __name__ == '__main__':
    ...
    sentiment_tweet_df = get_sentiment_from_file()
    price_df = get_bitcoin_price_from_tweetwise()
    test_feature, test_label = get_features_labels(sentiment_tweet_df, price_df, 'average count')
    # print test_feature
    # print test_label.count(1), len(test_label)
    save_features_to_file(test_feature, 'Features/average_count')
    # save_label_to_file(test_label)

    test_feature_load = load_features_from_file('Features/average_count', type_func=float)
    test_label_load = load_label_from_file()
    print((test_feature_load > test_feature) - (test_feature_load < test_feature))
    print((test_label_load > test_label) - (test_label_load < test_label))
